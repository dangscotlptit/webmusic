...
const fileFilter = (req, file, cb) => {
  if (file.mimetype.split('/')[0] === 'image') {
    ...
    cb(new Error('Chỉ chấp nhận hình ảnh'));
  }
};
...
exports.getPlaylist = catchAsync(async (req, res, next) => {
  ...
  if (!playlist)
    return next(new AppError('Không tồn tại danh sách', 404));
...
exports.updatePlaylist = catchAsync(async (req, res, next) => {
  ...
  if (!playlist)
    return next(new AppError('Không tìm thấy danh sách', 404));
...
exports.addSong = catchAsync(async (req, res, next) => {
  ...
  res.status(200).json({
    status: 'success',
    message: `Ca khúc được thêm vào ${playlist.name}`,
  });
});
